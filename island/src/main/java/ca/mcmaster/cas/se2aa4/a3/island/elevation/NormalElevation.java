...
public class NormalElevation implements Elevation {
    ...
    static final TreeMap<Double, ElevationLevels> ELEVATION_RANGES;
    ...
    private ElevationLevels assignLevel(double normDist) {
        for (Map.Entry<Double, ElevationLevels> rangeEntry : ELEVATION_RANGES.entrySet()) {
        ...
        for (int index : collection.getAllTileIdxs()) {
            if (collection.isOceanTile(index)) {
                ...
                for (Integer idx : shores) {
                    elev = Math.max(Math.min(
                            calculateDistanceDistribution(collection.getCentreX(index), collection.getCentreY(index),
                                    collection.getCentreX(idx), collection.getCentreY(idx)),
